...
namespace SharpKit.JavaScript.Private
{
	...
	internal partial class JsImplList<T> : IList<T>, IList
	{
        ...
        [JsMethod(Name = "IndexOf$$T")]
		public int IndexOf(T item)
		{
			return _list.indexOf(item);
		}

        public List<TOut> ConvertAll<TOut>(Func<T, TOut> converter)
        {
            List<TOut> result = new List<TOut>();
            for (int i = 0; i < this._list.length; i++)
                result.Add(converter(this._list[i].As<T>()));
            return result;
        }
        ...
        {
            T item;
            for (int i = 0, j = this._list.length; i < j; i++)
            {
                item = this._list[i].As<T>();
                if (pred(item))
                    return item;
            }
            return default(T);
        ...
        [JsMethod(Name = "FindIndex$$Predicate$1")]
