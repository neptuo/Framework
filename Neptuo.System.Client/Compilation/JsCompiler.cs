...
namespace Neptuo.Client.Compilation
{
    ...
    public class JsCompiler : JsCompilerGlobal
    {
        ...
        public static JsArray<Type> NewTypes = new JsArray<Type>();
        public static JsArray<JsAction> AfterCompile = new JsArray<JsAction>();
        ...
        private static void Compile_Phase2_TmpType(JsType tmpType)
        {
            ...
            {
                bool result = CopyMemberIfNotDefined(type, type.fullname, window); //window[type.get_FullName()] = type;//.ctor;
                if (result)
        ...
        private static void Compile_Phase3()
        {
            ...
            for (int i = 0; i < AfterCompile.length; i++)
                AfterCompile[i]();
        }

        #region JsCode

        [JsMethod(NativeOverloads = true, Code = @"if(target[name]===undefined) { target[name] = source; return true; } else { return false; }")]
        internal static bool CopyMemberIfNotDefined(object source, string name, object target)
        {
            return false;
        }
